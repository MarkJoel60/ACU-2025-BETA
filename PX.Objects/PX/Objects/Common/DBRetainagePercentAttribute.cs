...
// Type: PX.Objects.Common.DBRetainagePercentAttribute
...
public class DBRetainagePercentAttribute : RetainagePercentAttribute
{
  ...
  {
    this._Attributes.Add((PXEventSubscriberAttribute) new PXDBDecimalAttribute(6)
    {
      MinValue = 0.0,
      MaxValue = 100.0
    });
    this._Attributes.Add((PXEventSubscriberAttribute) new PXDefaultAttribute(TypeCode.Decimal, "0.0", this.defaultType)
    {
      PersistingCheck = (PXPersistingCheck) 2
    });
    this._Attributes.Add((PXEventSubscriberAttribute) new PXUIVerifyAttribute(this.verifyType, (PXErrorLevel) 4, "A retainage percent must be between 0 and 100.", Array.Empty<Type>()));
