...
// Type: PX.Objects.FA.DepreciationMethods.NewZealandStraightLineMethod
...
public class NewZealandStraightLineMethod : NewZealandMethodBase
{
  protected override string CalculationMethod => "ZL";
  ...
  protected override SortedDictionary<string, SortedDictionary<string, DepreciationMethodBase.FADepreciationScheduleItem>> Calculate()
  {
    ...
    foreach (FAAddition addition in this.CalculationParameters.Additions)
    {
      ...
      Decimal num1 = 0M;
      Decimal num2 = 0M;
      Decimal num3 = additionParameters.DepreciationBasis * this.CalculationParameters.BookBalance.PercentPerYear.Value / 100M;
      Decimal num4 = 0M;
      Decimal num5 = 0M;
      Decimal num6 = 0M;
      NewZealandStraightLineMethod.FinYearInfo finYearInfo1 = (NewZealandStraightLineMethod.FinYearInfo) null;
      IYearSetup yearSetup = this.IncomingParameters.PeriodDepreciationUtils.YearSetup;
      Dictionary<string, NewZealandStraightLineMethod.FinYearInfo> finYearInfo2 = this.GetFinYearInfo(additionPeriods, yearSetup);
      ...
      foreach (string str in additionPeriods)
      {
        if (string.CompareOrdinal(str, this.CalculationParameters.MaxDepreciateToPeriodID) <= 0)
        {
          ...
          Decimal num7 = !(str == period) ? (!(str == finYearInfo1.LastFinPeriodOfFinYear) ? num3 * (num6 / num5) * (daysHeldInPeriod / num4) : num3 * (num6 / num5) - num2) : additionParameters.DepreciationBasis - num1;
  ...
  private Dictionary<string, NewZealandStraightLineMethod.FinYearInfo> GetFinYearInfo(
    List<string> periods,
    IYearSetup yearSetup)
  {
    Dictionary<string, NewZealandStraightLineMethod.FinYearInfo> finYearInfo1 = new Dictionary<string, NewZealandStraightLineMethod.FinYearInfo>();
    foreach (string period in periods)
    {
      FABookPeriod bookPeriod = this.IncomingParameters.PeriodDepreciationUtils.BookPeriods[period];
      NewZealandStraightLineMethod.FinYearInfo finYearInfo2;
      if (finYearInfo1.TryGetValue(bookPeriod.FinYear, out finYearInfo2))
      {
        ...
        finYearInfo1[bookPeriod.FinYear] = new NewZealandStraightLineMethod.FinYearInfo()
        {
  ...
  private void RecalculateYearInfoForDisposalPeriod(
    Dictionary<string, NewZealandStraightLineMethod.FinYearInfo> map,
    ...
    IYearSetup yearSetup)
  {
    ...
    NewZealandStraightLineMethod.FinYearInfo finYearInfo;
