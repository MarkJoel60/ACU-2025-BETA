...
// Type: PX.Objects.IN.ReasonCodeSubAccountMaskAttribute
...
public sealed class ReasonCodeSubAccountMaskAttribute : PXEntityAttribute
{
  ...
  {
    PXDimensionMaskAttribute dimensionMaskAttribute = !PXAccess.FeatureInstalled<FeaturesSet.projectAccounting>() ? new PXDimensionMaskAttribute("SUBACCOUNT", "ReasonCodeIN", "R", ReasonCodeSubAccountMaskAttribute.writeOffValuesWithoutProjects, ReasonCodeSubAccountMaskAttribute.writeOffLabelsWithoutProjects) : new PXDimensionMaskAttribute("SUBACCOUNT", "ReasonCodeIN", "R", ReasonCodeSubAccountMaskAttribute.writeOffValues, ReasonCodeSubAccountMaskAttribute.writeOffLabels);
  ...
  public static string MakeSub<Field>(PXGraph graph, string mask, object[] sources, Type[] fields) where Field : IBqlField
  {
    try
    {
      return PXDimensionMaskAttribute.MakeSub<Field>(graph, mask, ReasonCodeSubAccountMaskAttribute.writeOffValues, sources);
    }
    catch (PXMaskArgumentException ex)
    {
      ...
      {
        (object) ReasonCodeSubAccountMaskAttribute.writeOffLabels[ex.SourceIdx],
