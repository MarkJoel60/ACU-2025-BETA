...
// Type: PX.Objects.IN.GraphExtensions.QuoteMaintExt.MatrixEntryExt
...
public class MatrixEntryExt : SmartPanelExt<QuoteMaint, CRQuote>
{
  ...
  protected override void CreateNewLine(
    ...
    string uom)
  {
    CROpportunityProducts opportunityProducts1 = ((PXSelectBase<CROpportunityProducts>) this.Base.Products).Insert();
    if (opportunityProducts1 == null)
    {
      int num = (int?) CROpportunity.PK.Find((PXGraph) this.Base, ((PXSelectBase<CRQuote>) this.Base.Quote).Current?.OpportunityID)?.ProductCntr ?? 1;
      for (int index = 0; index <= num; ++index)
      {
        opportunityProducts1 = ((PXSelectBase<CROpportunityProducts>) this.Base.Products).Insert();
        if (opportunityProducts1 != null)
          break;
      }
    }
    CROpportunityProducts copy1 = PXCache<CROpportunityProducts>.CreateCopy(opportunityProducts1);
    copy1.SiteID = siteID;
    ...
    if (uom != null)
      copy2.UOM = uom;
    CROpportunityProducts opportunityProducts2 = ((PXSelectBase<CROpportunityProducts>) this.Base.Products).Update(copy2);
    if (string.IsNullOrEmpty(taxCategoryID))
      return;
    ((PXSelectBase) this.Base.Products).Cache.SetValueExt<CROpportunityProducts.taxCategoryID>((object) opportunityProducts2, (object) taxCategoryID);
    ((PXSelectBase<CROpportunityProducts>) this.Base.Products).Update(opportunityProducts2);
  }
  ...
  {
    return ((PXSelectBase<CRQuote>) this.Base.Quote).Current?.BranchID;
  }
}
