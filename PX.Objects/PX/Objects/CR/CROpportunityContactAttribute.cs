...
// Type: PX.Objects.CR.CROpportunityContactAttribute
...
public class CROpportunityContactAttribute(System.Type SelectType) : CRContactAttribute(typeof (CRContact.contactID), typeof (CRContact.isDefaultContact), SelectType)
{
  public override void CacheAttached(PXCache sender)
  {
    base.CacheAttached(sender);
    PXGraph.FieldVerifyingEvents fieldVerifying = sender.Graph.FieldVerifying;
    CROpportunityContactAttribute contactAttribute = this;
    ...
    fieldVerifying.AddHandler<CRContact.overrideContact>(pxFieldVerifying);
  }
    ...
    object ContactRow)
  {
    ...
    if (sender.GetValue<CROpportunity.contactID>(DocumentRow) != null)
    {
      ...
      BqlCommand bqlCommand = (BqlCommand) new SelectFromBase<Contact, TypeArrayOf<IFbqlJoin>.Append<TypeArrayOf<IFbqlJoin>.Empty, FbqlJoins.Left<CRContact>.On<BqlChainableConditionBase<TypeArrayOf<IBqlBinary>.Append<TypeArrayOf<IBqlBinary>.Append<TypeArrayOf<IBqlBinary>.FilledWith<And<Compare<CRContact.bAccountID, Equal<Contact.bAccountID>>>>, And<BqlOperand<CRContact.bAccountContactID, IBqlInt>.IsEqual<Contact.contactID>>>, And<BqlOperand<CRContact.revisionID, IBqlInt>.IsEqual<Contact.revisionID>>>>.And<BqlOperand<CRContact.isDefaultContact, IBqlBool>.IsEqual<boolTrue>>>>>.Where<BqlOperand<Contact.contactID, IBqlInt>.IsEqual<P.AsInt>>();
    ...
    else if (sender.GetValue<CROpportunity.bAccountID>(DocumentRow) != null)
    {
      ...
      BqlCommand bqlCommand = (BqlCommand) new SelectFromBase<Contact, TypeArrayOf<IFbqlJoin>.Append<TypeArrayOf<IFbqlJoin>.Append<TypeArrayOf<IFbqlJoin>.Empty, FbqlJoins.Left<BAccount>.On<BqlOperand<BAccount.defContactID, IBqlInt>.IsEqual<Contact.contactID>>>, FbqlJoins.Left<CRContact>.On<BqlChainableConditionBase<TypeArrayOf<IBqlBinary>.Append<TypeArrayOf<IBqlBinary>.Append<TypeArrayOf<IBqlBinary>.FilledWith<And<Compare<CRContact.bAccountID, Equal<Contact.bAccountID>>>>, And<BqlOperand<CRContact.bAccountContactID, IBqlInt>.IsEqual<Contact.contactID>>>, And<BqlOperand<CRContact.revisionID, IBqlInt>.IsEqual<Contact.revisionID>>>>.And<BqlOperand<CRContact.isDefaultContact, IBqlBool>.IsEqual<boolTrue>>>>>.Where<BqlOperand<BAccount.bAccountID, IBqlInt>.IsEqual<P.AsInt>>();
      pxView = sender.Graph.TypedViews.GetView(bqlCommand, false);
    }
    else if (sender.GetValue<CROpportunity.locationID>(DocumentRow) != null)
    {
      ...
      BqlCommand bqlCommand = (BqlCommand) new SelectFromBase<Contact, TypeArrayOf<IFbqlJoin>.Append<TypeArrayOf<IFbqlJoin>.Append<TypeArrayOf<IFbqlJoin>.Empty, FbqlJoins.Left<Location>.On<BqlOperand<Location.defContactID, IBqlInt>.IsEqual<Contact.contactID>>>, FbqlJoins.Left<CRContact>.On<BqlChainableConditionBase<TypeArrayOf<IBqlBinary>.Append<TypeArrayOf<IBqlBinary>.Append<TypeArrayOf<IBqlBinary>.FilledWith<And<Compare<CRContact.bAccountID, Equal<Contact.bAccountID>>>>, And<BqlOperand<CRContact.bAccountContactID, IBqlInt>.IsEqual<Contact.contactID>>>, And<BqlOperand<CRContact.revisionID, IBqlInt>.IsEqual<Contact.revisionID>>>>.And<BqlOperand<CRContact.isDefaultContact, IBqlBool>.IsEqual<boolTrue>>>>>.Where<BqlOperand<Location.locationID, IBqlInt>.IsEqual<P.AsInt>>();
      pxView = sender.Graph.TypedViews.GetView(bqlCommand, false);
    }
  ...
  {
    this.DefaultContact<CRContact, CRContact.contactID>(sender, DocumentRow, Row);
  }
  ...
  public override void CopyRecord(
    ...
    bool clone)
  {
    this.CopyContact<CRContact, CRContact.contactID>(sender, DocumentRow, SourceRow, clone);
  }
  ...
  public virtual void Record_Override_FieldVerifying(PXCache sender, PXFieldVerifyingEventArgs e)
  {
    ...
    try
    {
      this.Contact_IsDefaultContact_FieldVerifying<CRContact>(sender, e);
    }
    finally
    {
