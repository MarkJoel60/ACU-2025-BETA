...
// Type: PX.Objects.CR.Extensions.CRDuplicateEntities.CRDuplicateLeadsSelectorAttribute
...
public class CRDuplicateLeadsSelectorAttribute : PXCustomSelectorAttribute
{
  ...
  public CRDuplicateLeadsSelectorAttribute(System.Type sourceEntityID)
    : base(typeof (FbqlSelect<SelectFromBase<CRLead, TypeArrayOf<IFbqlJoin>.Append<TypeArrayOf<IFbqlJoin>.Append<TypeArrayOf<IFbqlJoin>.Append<TypeArrayOf<IFbqlJoin>.Empty, FbqlJoins.Inner<CRDuplicateGrams>.On<BqlOperand<CRDuplicateGrams.entityID, IBqlInt>.IsEqual<CRLead.contactID>>>, FbqlJoins.Inner<CRGrams>.On<BqlChainableConditionBase<TypeArrayOf<IBqlBinary>.FilledWith<And<Compare<CRGrams.validationType, Equal<CRDuplicateGrams.validationType>>>>>.And<BqlChainableConditionBase<TypeArrayOf<IBqlBinary>.FilledWith<And<Compare<CRGrams.entityName, Equal<CRDuplicateGrams.entityName>>>>>.And<BqlChainableConditionBase<TypeArrayOf<IBqlBinary>.FilledWith<And<Compare<CRGrams.fieldName, Equal<CRDuplicateGrams.fieldName>>>>>.And<BqlOperand<CRGrams.fieldValue, IBqlString>.IsEqual<CRDuplicateGrams.fieldValue>>>>>>, FbqlJoins.Left<CRValidation>.On<BqlOperand<CRValidation.type, IBqlString>.IsEqual<CRGrams.validationType>>>>.Where<BqlChainableConditionBase<TypeArrayOf<IBqlBinary>.FilledWith<And<Compare<CRGrams.entityID, Equal<BqlField<CRLead.contactID, IBqlInt>.AsOptional>>>>>.And<BqlOperand<CRDuplicateGrams.validationType, IBqlString>.IsEqual<ValidationTypesAttribute.leadToLead>>>.Aggregate<To<GroupBy<CRDuplicateGrams.entityID>, GroupBy<CRDuplicateGrams.validationType>, GroupBy<CRGrams.entityID>, Sum<CRGrams.score>>>.Having<BqlAggregatedOperand<Sum<CRGrams.score>, IBqlDecimal>.IsGreaterEqual<BqlField<CRValidation.validationThreshold, IBqlDecimal>.Maximized>>, CRLead>.SearchFor<CRLead.contactID>), new System.Type[4]
    {
    ...
    })
  {
    ...
    ((PXSelectorAttribute) this).DescriptionField = typeof (CRLead.displayName);
  ...
  {
    CRDuplicateLeadsSelectorAttribute selectorAttribute = this;
    ...
    if (obj != null)
    {
      ...
      {
        CRLead record = pxResult.GetItem<CRLead>();
