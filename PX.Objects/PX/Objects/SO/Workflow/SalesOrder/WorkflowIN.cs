...
// Type: PX.Objects.SO.Workflow.SalesOrder.WorkflowIN
...
public class WorkflowIN : WorkflowBase
{
  ...
  {
    WorkflowIN.Configure(config.GetScreenConfigurationContext<SOOrderEntry, SOOrder>());
  }
  ...
  {
    public BoundedTo<SOOrderEntry, SOOrder>.Condition HasAllBillsReleased
    {
      get
      {
        return this.GetOrCreate((Func<BoundedTo<SOOrderEntry, SOOrder>.Condition.ConditionBuilder, BoundedTo<SOOrderEntry, SOOrder>.Condition>) (b => b.FromBql<BqlChainableConditionBase<TypeArrayOf<IBqlBinary>.FilledWith<And<Compare<SOOrder.billedCntr, Equal<Zero>>>>>.And<BqlOperand<SOOrder.releasedCntr, IBqlInt>.IsGreater<Zero>>>()), nameof (HasAllBillsReleased));
      }
    }

    public BoundedTo<SOOrderEntry, SOOrder>.Condition IsUnbilled
    {
      get
      {
        return this.GetOrCreate((Func<BoundedTo<SOOrderEntry, SOOrder>.Condition.ConditionBuilder, BoundedTo<SOOrderEntry, SOOrder>.Condition>) (b => b.FromBql<BqlChainableConditionBase<TypeArrayOf<IBqlBinary>.FilledWith<And<Compare<SOOrder.billedCntr, Equal<Zero>>>>>.And<BqlOperand<SOOrder.releasedCntr, IBqlInt>.IsEqual<Zero>>>()), nameof (IsUnbilled));
      }
    }
  }
}
