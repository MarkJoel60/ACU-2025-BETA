...
public class RateEngine
{
  ...
  public RateEngine(PXGraph graph, string rateTypeID, PMTran tran)
  {
    ...
    if (string.IsNullOrEmpty(rateTypeID))
      throw new ArgumentNullException(nameof (rateTypeID), PXMessages.LocalizeNoPrefix("Argument is null or an empty string."));
    if (tran == null)
  ...
  public Decimal? GetRate(string rateTableID)
  {
    ...
    this.trace.AppendFormat("Calculating Rate. RateTable:{0}", (object) rateTableID);
    foreach (PXResult<PMRateDefinition> pxResult in ((PXSelectBase<PMRateDefinition>) new PXSelect<PMRateDefinition, Where<PMRateDefinition.rateTableID, Equal<Required<PMRateDefinition.rateTableID>>, And<PMRateDefinition.rateTypeID, Equal<Required<PMRateDefinition.rateTypeID>>>>, OrderBy<Asc<PMRateDefinition.sequence>>>(this.graph)).Select(new object[2]
    {
      (object) rateTableID,
      (object) this.rateTypeID
    }))
    {
      PMRateDefinition rd = PXResult<PMRateDefinition>.op_Implicit(pxResult);
      this.trace.AppendFormat("Start Processing Sequence:{0}", (object) rd.Description);
      Decimal? rate = this.GetRate(rd);
      if (rate.HasValue)
      {
  ...
  {
    PMAccountGroup pmAccountGroup = PMAccountGroup.PK.Find(this.graph, this.tran.AccountGroupID);
  ...
  protected virtual Decimal? GetRate(PMRateDefinition rd)
  {
    ...
    if (rd.Project.GetValueOrDefault())
    {
      ...
      if (!this.IsProjectFit(rd.RateDefinitionID, this.tran.ProjectID, out rateCodes))
        flag = false;
      else
        first = rateCodes;
    }
    ...
    {
      List<string> rateCodes;
      if (!this.IsTaskFit(rd.RateDefinitionID, this.tran.ProjectID, this.tran.TaskID, out rateCodes))
        flag = false;
      else if (first == null)
      {
        first = rateCodes;
    ...
    {
      List<string> rateCodes;
      if (!this.IsAccountGroupFit(rd.RateDefinitionID, this.tran.AccountGroupID, out rateCodes))
        flag = false;
      else if (first == null)
      {
        first = rateCodes;
    ...
    {
      List<string> rateCodes;
      if (!this.IsItemFit(rd.RateDefinitionID, this.tran.InventoryID, out rateCodes))
        flag = false;
      else if (first == null)
      {
        first = rateCodes;
    ...
    {
      List<string> rateCodes;
      if (!this.IsEmployeeFit(rd.RateDefinitionID, this.tran.ResourceID, out rateCodes))
        flag = false;
      else if (first == null)
      {
        first = rateCodes;
    ...
    this.trace.AppendFormat("\tSearching Rate for Date:{0}", (object) this.tran.Date);
    return pmRate?.Rate;
  }
