...
// Type: PX.Objects.CA.CAAPAROpenPeriodAttribute
...
public class CAAPAROpenPeriodAttribute : OpenPeriodAttribute
{
  ...
  public CAAPAROpenPeriodAttribute(
    Type origModule,
    Type sourceType,
    ...
    : base(((IBqlTemplate) BqlTemplate.OfCommand<Search<FinPeriod.finPeriodID, Where<FinPeriod.status, Equal<FinPeriod.status.open>, And<Where2<Where<Current<CAAPAROpenPeriodAttribute.OrigModulePh>, Equal<BatchModule.moduleCA>, And<FinPeriod.cAClosed, Equal<False>>>, Or2<Where<Current<CAAPAROpenPeriodAttribute.OrigModulePh>, Equal<BatchModule.moduleAP>, And<FinPeriod.aPClosed, Equal<False>>>, Or2<Where<Current<CAAPAROpenPeriodAttribute.OrigModulePh>, Equal<BatchModule.moduleAR>, And<FinPeriod.aRClosed, Equal<False>>>, Or<Current<CAAPAROpenPeriodAttribute.OrigModulePh>, IsNull>>>>>>>>.Replace<CAAPAROpenPeriodAttribute.OrigModulePh>(origModule)).ToType(), sourceType, branchSourceType, branchSourceFormulaType, organizationSourceType, useMasterCalendarSourceType, defaultType, redefaultOrRevalidateOnOrganizationSourceUpdated, masterFinPeriodIDType: masterFinPeriodIDType, redefaultOnDateChanged: redefaultOnDateChanged)
  {
  ...
  {
    foreach (OpenPeriodAttribute openPeriodAttribute in cache.GetAttributesReadonly<Field>(row).OfType<CAAPAROpenPeriodAttribute>())
    ...
    FinPeriod finPeriod)
  {
    ...
    if (!validationResult.HasWarningOrError)
    {
      ...
      if (str == "CA" && finPeriod.CAClosed.GetValueOrDefault() || str == "AP" && finPeriod.APClosed.GetValueOrDefault() || str == "AR" && finPeriod.ARClosed.GetValueOrDefault())
