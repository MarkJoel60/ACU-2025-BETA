// Decompiled with JetBrains decompiler
// Type: PX.Objects.CC.PayLinkPaymentMethod
// Assembly: PX.Objects, Version=1.0.0.0, Culture=neutral, PublicKeyToken=null
// MVID: CF76B6BF-0C8A-413D-8225-C21BEAE6CEEC
// Assembly location: D:\tmp\2025 R2 DLLs\PX.Objects.dll
// XML documentation location: D:\tmp\2025 R2 DLLs\PX.Objects.xml

using PX.Data;
using System;

#nullable disable
namespace PX.Objects.CC;

public static class PayLinkPaymentMethod
{
  public const string NotSpecified = "N";
  public const string Eft = "E";
  public const string CreditCard = "C";

  public static Tuple<string, string>[] ValueLabelPairs()
  {
    return new Tuple<string, string>[3]
    {
      new Tuple<string, string>("N", "EFT & Credit Card"),
      new Tuple<string, string>("C", "Credit Card"),
      new Tuple<string, string>("E", "EFT")
    };
  }

  public class ListAttribute : PXStringListAttribute
  {
    public ListAttribute()
      : base(PayLinkPaymentMethod.ValueLabelPairs())
    {
    }
  }
}
