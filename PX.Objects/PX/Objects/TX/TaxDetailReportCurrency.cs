// Decompiled with JetBrains decompiler
// Type: PX.Objects.TX.TaxDetailReportCurrency
// Assembly: PX.Objects, Version=1.0.0.0, Culture=neutral, PublicKeyToken=null
// MVID: CF76B6BF-0C8A-413D-8225-C21BEAE6CEEC
// Assembly location: D:\tmp\2025 R2 DLLs\PX.Objects.dll
// XML documentation location: D:\tmp\2025 R2 DLLs\PX.Objects.xml

using PX.Data;
using PX.Data.BQL;
using PX.Objects.AP;
using PX.Objects.CM;
using PX.Objects.CM.Extensions;
using PX.Objects.GL;
using System;

#nullable enable
namespace PX.Objects.TX;

[PXProjection(typeof (Select2<TaxPeriodEffective, InnerJoin<TaxReportLine, On<TaxReportLine.vendorID, Equal<TaxPeriodEffective.vendorID>>, InnerJoin<TaxBucketLine, On<TaxBucketLine.vendorID, Equal<TaxReportLine.vendorID>, And<TaxBucketLine.taxReportRevisionID, Equal<TaxReportLine.taxReportRevisionID>, And<TaxBucketLine.lineNbr, Equal<TaxReportLine.lineNbr>>>>, CrossJoin<Company, InnerJoin<PX.Objects.AP.Vendor, On<PX.Objects.AP.Vendor.bAccountID, Equal<TaxReportLine.vendorID>>, LeftJoin<TaxTran, On<TaxTran.vendorID, Equal<TaxBucketLine.vendorID>, And<TaxTran.taxBucketID, Equal<TaxBucketLine.bucketID>, And2<Where<TaxReportLine.taxZoneID, IsNull, And<TaxReportLine.tempLine, Equal<False>, Or<TaxReportLine.taxZoneID, Equal<TaxTran.taxZoneID>>>>, And<Where<TaxTran.taxPeriodID, IsNull, And<TaxTran.origRefNbr, Equal<Empty>, And<TaxTran.released, Equal<True>, And<TaxTran.voided, Equal<False>, And<TaxTran.taxType, NotEqual<PX.Objects.TX.TaxType.pendingSales>, And<TaxTran.taxType, NotEqual<PX.Objects.TX.TaxType.pendingPurchase>, And<TaxTran.tranDate, Less<TaxPeriodEffective.endDate>, And<TaxPeriodEffective.status, Equal<TaxPeriodStatus.open>, Or<TaxTran.taxPeriodID, Equal<TaxPeriodEffective.taxPeriodID>>>>>>>>>>>>>>, LeftJoin<PX.Objects.CM.Extensions.CurrencyInfo, On<PX.Objects.CM.Extensions.CurrencyInfo.curyInfoID, Equal<TaxTran.curyInfoID>>, LeftJoin<CurrencyRateByDate, On<CurrencyRateByDate.fromCuryID, Equal<PX.Objects.CM.Extensions.CurrencyInfo.curyID>, And<CurrencyRateByDate.toCuryID, Equal<PX.Objects.AP.Vendor.curyID>, And<CurrencyRateByDate.curyRateType, Equal<PX.Objects.AP.Vendor.curyRateTypeID>, And<CurrencyRateByDate.curyEffDate, LessEqual<TaxTran.tranDate>, And<Where<CurrencyRateByDate.nextEffDate, Greater<TaxTran.tranDate>, Or<CurrencyRateByDate.nextEffDate, IsNull>>>>>>>>>>>>>>>))]
[Obsolete("TaxDetailReportCurrency is obsolete and will be removed in Acumatica 8.0.")]
[PXCacheName("Tax Detail Report Currency")]
public class TaxDetailReportCurrency : PXBqlTable, IBqlTable, IBqlTableSystemDataStorage
{
  protected int? _LineNbr;
  protected short? _LineMult;
  protected 
  #nullable disable
  string _LineType;
  protected string _Module;
  protected string _TranType;
  protected string _RefNbr;
  protected int? _RecordID;
  protected bool? _Released;
  protected bool? _Voided;
  protected string _TaxPeriodID;
  protected string _TaxID;
  protected int? _VendorID;
  protected int? _BranchID;
  protected string _TaxZoneID;
  protected int? _AccountID;
  protected int? _SubID;
  protected DateTime? _TranDate;
  protected string _TaxType;
  protected Decimal? _TaxRate;
  protected Decimal? _TaxableAmt;
  protected Decimal? _TaxAmt;
  protected string _VendorCuryID;
  protected string _VendorCuryRateType;
  protected Decimal? _CuryRate;
  protected string _CuryMultDiv;

  [PXDBInt(IsKey = true, BqlField = typeof (TaxReportLine.lineNbr))]
  public virtual int? LineNbr
  {
    get => this._LineNbr;
    set => this._LineNbr = value;
  }

  [PXDBShort(BqlField = typeof (TaxReportLine.lineMult))]
  public virtual short? LineMult
  {
    get => this._LineMult;
    set => this._LineMult = value;
  }

  [PXDBString(1, IsFixed = true, BqlField = typeof (TaxReportLine.lineType))]
  public virtual string LineType
  {
    get => this._LineType;
    set => this._LineType = value;
  }

  [PXDBString(2, IsKey = true, IsFixed = true, BqlField = typeof (TaxTran.module))]
  public virtual string Module
  {
    get => this._Module;
    set => this._Module = value;
  }

  [PXDBString(3, IsKey = true, IsFixed = true, BqlField = typeof (TaxTran.tranType))]
  [TaxAdjustmentType.List]
  public virtual string TranType
  {
    get => this._TranType;
    set => this._TranType = value;
  }

  [PXDBString(15, IsKey = true, BqlField = typeof (TaxTran.refNbr))]
  public virtual string RefNbr
  {
    get => this._RefNbr;
    set => this._RefNbr = value;
  }

  [PXDBInt(IsKey = true, BqlField = typeof (TaxTran.recordID))]
  public virtual int? RecordID
  {
    get => this._RecordID;
    set => this._RecordID = value;
  }

  [PXDBBool(BqlField = typeof (TaxTran.released))]
  public virtual bool? Released
  {
    get => this._Released;
    set => this._Released = value;
  }

  [PXDBBool(BqlField = typeof (TaxTran.voided))]
  public virtual bool? Voided
  {
    get => this._Voided;
    set => this._Voided = value;
  }

  [FinPeriodID(null, null, null, null, null, null, true, false, null, null, null, true, true, BqlField = typeof (TaxPeriodEffective.taxPeriodID))]
  public virtual string TaxPeriodID
  {
    get => this._TaxPeriodID;
    set => this._TaxPeriodID = value;
  }

  [PXDBString(60, IsKey = true, BqlField = typeof (TaxTran.taxID))]
  public virtual string TaxID
  {
    get => this._TaxID;
    set => this._TaxID = value;
  }

  [Vendor(BqlField = typeof (TaxTran.vendorID))]
  public virtual int? VendorID
  {
    get => this._VendorID;
    set => this._VendorID = value;
  }

  [Branch(null, null, true, true, true, BqlField = typeof (TaxTran.branchID))]
  public virtual int? BranchID
  {
    get => this._BranchID;
    set => this._BranchID = value;
  }

  [PXDBString(10, BqlField = typeof (TaxTran.taxZoneID))]
  public virtual string TaxZoneID
  {
    get => this._TaxZoneID;
    set => this._TaxZoneID = value;
  }

  [Account(null, BqlField = typeof (TaxTran.accountID))]
  public virtual int? AccountID
  {
    get => this._AccountID;
    set => this._AccountID = value;
  }

  [SubAccount(typeof (TaxTran.accountID), BqlField = typeof (TaxTran.subID))]
  public virtual int? SubID
  {
    get => this._SubID;
    set => this._SubID = value;
  }

  [PXDBDate(BqlField = typeof (TaxTran.tranDate))]
  public virtual DateTime? TranDate
  {
    get => this._TranDate;
    set => this._TranDate = value;
  }

  [PXDBString(1, IsFixed = true, BqlField = typeof (TaxTran.taxType))]
  public virtual string TaxType
  {
    get => this._TaxType;
    set => this._TaxType = value;
  }

  [PXDBDecimal(6, BqlField = typeof (TaxTran.taxRate))]
  public virtual Decimal? TaxRate
  {
    get => this._TaxRate;
    set => this._TaxRate = value;
  }

  [PXDBBaseCury(BqlField = typeof (TaxTran.taxableAmt))]
  public virtual Decimal? TaxableAmt
  {
    [PXDependsOnFields(new Type[] {typeof (TaxDetailReportCurrency.module), typeof (TaxDetailReportCurrency.tranType), typeof (TaxDetailReportCurrency.taxType), typeof (TaxDetailReportCurrency.lineMult)})] get
    {
      Decimal mult = ReportTaxProcess.GetMult(this._Module, this._TranType, this._TaxType, this._LineMult);
      Decimal? taxableAmt = this._TaxableAmt;
      return !taxableAmt.HasValue ? new Decimal?() : new Decimal?(mult * taxableAmt.GetValueOrDefault());
    }
    set => this._TaxableAmt = value;
  }

  [PXDBBaseCury(BqlField = typeof (TaxTran.taxAmt))]
  public virtual Decimal? TaxAmt
  {
    [PXDependsOnFields(new Type[] {typeof (TaxDetailReportCurrency.module), typeof (TaxDetailReportCurrency.tranType), typeof (TaxDetailReportCurrency.taxType), typeof (TaxDetailReportCurrency.lineMult)})] get
    {
      Decimal mult = ReportTaxProcess.GetMult(this._Module, this._TranType, this._TaxType, this._LineMult);
      Decimal? taxAmt = this._TaxAmt;
      return !taxAmt.HasValue ? new Decimal?() : new Decimal?(mult * taxAmt.GetValueOrDefault());
    }
    set => this._TaxAmt = value;
  }

  [PXDBString(5, BqlField = typeof (PX.Objects.AP.Vendor.curyID))]
  public virtual string VendorCuryID
  {
    get => this._VendorCuryID;
    set => this._VendorCuryID = value;
  }

  [PXDBString(6, BqlField = typeof (PX.Objects.AP.Vendor.curyRateTypeID))]
  public virtual string VendorCuryRateType
  {
    get => this._VendorCuryRateType;
    set => this._VendorCuryRateType = value;
  }

  [PXDBDecimal(6, BqlField = typeof (CurrencyRateByDate.curyRate))]
  public virtual Decimal? CuryRate
  {
    get => this._CuryRate;
    set => this._CuryRate = value;
  }

  [PXDBString(1, BqlField = typeof (CurrencyRateByDate.curyMultDiv))]
  public virtual string CuryMultDiv
  {
    get => this._CuryMultDiv;
    set => this._CuryMultDiv = value;
  }

  public abstract class lineNbr : BqlType<
  #nullable enable
  IBqlInt, int>.Field<
  #nullable disable
  TaxDetailReportCurrency.lineNbr>
  {
  }

  public abstract class lineMult : BqlType<
  #nullable enable
  IBqlShort, short>.Field<
  #nullable disable
  TaxDetailReportCurrency.lineMult>
  {
  }

  public abstract class lineType : 
    BqlType<
    #nullable enable
    IBqlString, string>.Field<
    #nullable disable
    TaxDetailReportCurrency.lineType>
  {
  }

  public abstract class module : BqlType<
  #nullable enable
  IBqlString, string>.Field<
  #nullable disable
  TaxDetailReportCurrency.module>
  {
  }

  public abstract class tranType : 
    BqlType<
    #nullable enable
    IBqlString, string>.Field<
    #nullable disable
    TaxDetailReportCurrency.tranType>
  {
  }

  public abstract class refNbr : BqlType<
  #nullable enable
  IBqlString, string>.Field<
  #nullable disable
  TaxDetailReportCurrency.refNbr>
  {
  }

  public abstract class recordID : BqlType<
  #nullable enable
  IBqlInt, int>.Field<
  #nullable disable
  TaxDetailReportCurrency.recordID>
  {
  }

  public abstract class released : BqlType<
  #nullable enable
  IBqlBool, bool>.Field<
  #nullable disable
  TaxDetailReportCurrency.released>
  {
  }

  public abstract class voided : BqlType<
  #nullable enable
  IBqlBool, bool>.Field<
  #nullable disable
  TaxDetailReportCurrency.voided>
  {
  }

  public abstract class taxPeriodID : 
    BqlType<
    #nullable enable
    IBqlString, string>.Field<
    #nullable disable
    TaxDetailReportCurrency.taxPeriodID>
  {
  }

  public abstract class taxID : BqlType<
  #nullable enable
  IBqlString, string>.Field<
  #nullable disable
  TaxDetailReportCurrency.taxID>
  {
  }

  public abstract class vendorID : BqlType<
  #nullable enable
  IBqlInt, int>.Field<
  #nullable disable
  TaxDetailReportCurrency.vendorID>
  {
  }

  public abstract class branchID : BqlType<
  #nullable enable
  IBqlInt, int>.Field<
  #nullable disable
  TaxDetailReportCurrency.branchID>
  {
  }

  public abstract class taxZoneID : 
    BqlType<
    #nullable enable
    IBqlString, string>.Field<
    #nullable disable
    TaxDetailReportCurrency.taxZoneID>
  {
  }

  public abstract class accountID : BqlType<
  #nullable enable
  IBqlInt, int>.Field<
  #nullable disable
  TaxDetailReportCurrency.accountID>
  {
  }

  public abstract class subID : BqlType<
  #nullable enable
  IBqlInt, int>.Field<
  #nullable disable
  TaxDetailReportCurrency.subID>
  {
  }

  public abstract class tranDate : 
    BqlType<
    #nullable enable
    IBqlDateTime, DateTime>.Field<
    #nullable disable
    TaxDetailReportCurrency.tranDate>
  {
  }

  public abstract class taxType : BqlType<
  #nullable enable
  IBqlString, string>.Field<
  #nullable disable
  TaxDetailReportCurrency.taxType>
  {
  }

  public abstract class taxRate : 
    BqlType<
    #nullable enable
    IBqlDecimal, Decimal>.Field<
    #nullable disable
    TaxDetailReportCurrency.taxRate>
  {
  }

  public abstract class taxableAmt : 
    BqlType<
    #nullable enable
    IBqlDecimal, Decimal>.Field<
    #nullable disable
    TaxDetailReportCurrency.taxableAmt>
  {
  }

  public abstract class taxAmt : BqlType<
  #nullable enable
  IBqlDecimal, Decimal>.Field<
  #nullable disable
  TaxDetailReportCurrency.taxAmt>
  {
  }

  public abstract class vendorCuryID : 
    BqlType<
    #nullable enable
    IBqlString, string>.Field<
    #nullable disable
    TaxDetailReportCurrency.vendorCuryID>
  {
  }

  public abstract class vendorCuryRateType : 
    BqlType<
    #nullable enable
    IBqlString, string>.Field<
    #nullable disable
    TaxDetailReportCurrency.vendorCuryRateType>
  {
  }

  public abstract class curyRate : 
    BqlType<
    #nullable enable
    IBqlDecimal, Decimal>.Field<
    #nullable disable
    TaxDetailReportCurrency.curyRate>
  {
  }

  public abstract class curyMultDiv : 
    BqlType<
    #nullable enable
    IBqlString, string>.Field<
    #nullable disable
    TaxDetailReportCurrency.curyMultDiv>
  {
  }
}
