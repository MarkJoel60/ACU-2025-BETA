...
// Type: PX.Objects.GL.FinPeriods.IFinPeriodRepository
...
public interface IFinPeriodRepository
{
  ...
  FinPeriod FindMaxFinPeriodWithEndDataBelongToInterval(
    DateTime? startDate,
    DateTime? endDate,
    int? organizationID);
  ...
  /// <returns><see cref="T:PX.Objects.GL.FinPeriods.TableDefinition.FinPeriod" /></returns>
  ...
  IEnumerable<FinPeriod> GetFinPeriodsInInterval(
    DateTime? fromDate,
    DateTime? tillDate,
    int? organizationID);
  ...
  OrganizationFinPeriod FindLastNonAdjustmentOrganizationFinPeriodOfYear(
    int? organizationID,
    string finYear,
  ...
  IEnumerable<FinPeriod> PeriodsBetweenInclusive(
    DateTime startDate,
    DateTime endDate,
    int? organizationID);

  void CheckIsDateWithinPeriod(
    string finPeriodID,
    int? organizationID,
    DateTime date,
    string errorMessage);
  ...
  PX.Objects.GL.FinPeriods.TableDefinition.FinYear FindFirstYear(
    int? organizationID,
    bool clearQueryCache = false);

  PX.Objects.GL.FinPeriods.TableDefinition.FinYear FindLastYear(
    int? organizationID,
    bool clearQueryCache = false);
  ...
  OrganizationFinPeriod FindFirstOpenFinPeriod(
  ...
  ProcessingResult<FinPeriod> GetFinPeriodByMasterPeriodID(
    int? organizationID,
    string masterFinPeriodID);

  string FindFinPeriodIDByMasterPeriodID(
    int? organizationID,
    string masterFinPeriodID,
  ...
  OrganizationFinYear FindOrganizationFinYearByID(
    int? organizationID,
    string year,
    bool clearQueryCache = false);

  OrganizationFinPeriod FindOrganizationFinPeriodByID(
    int? organizationID,
    string finPeriodID,
    bool clearQueryCache = false);
  ...
  OrganizationFinYear FindNearestOrganizationFinYear(
    int? organizationID,
    string yearNumber,
    bool clearQueryCache = false,
