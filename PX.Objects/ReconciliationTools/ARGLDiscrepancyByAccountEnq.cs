...
// Type: ReconciliationTools.ARGLDiscrepancyByAccountEnq
...
public class ARGLDiscrepancyByAccountEnq : 
  ARGLDiscrepancyEnqGraphBase<ARGLDiscrepancyByAccountEnq, ARGLDiscrepancyEnqFilter, DiscrepancyByAccountEnqResult>
{
  ...
  [PXCustomizeBaseAttribute(typeof (PXUIFieldAttribute), "DisplayName", "AR Turnover")]
  protected virtual void DiscrepancyByAccountEnqResult_XXTurnover_CacheAttached(PXCache sender)
  {
  }

  [PXCustomizeBaseAttribute(typeof (PXUIFieldAttribute), "DisplayName", "Non-AR Transactions")]
  ...
  protected override List<DiscrepancyByAccountEnqResult> SelectDetails()
  {
    ...
    ARGLDiscrepancyEnqFilter header = this.Filter.Current;
    if (header != null)
    {
      ...
      if (nullable1.HasValue && header.PeriodFrom != null && header.PeriodTo != null)
      {
        ...
        if (nullable1.HasValue)
        {
          ...
          foreach (PXResult<GLTranR> pxResult in instance1.GLTranEnq.Select())
          {
            ...
            int num1;
            if (glTranR.Module == "AR" && !string.IsNullOrEmpty(glTranR.TranType) && !string.IsNullOrEmpty(glTranR.RefNbr))
            {
          ...
          ARCustomerBalanceEnq instance2 = PXGraph.CreateInstance<ARCustomerBalanceEnq>();
          ARCustomerBalanceEnq.ARHistoryFilter arHistoryFilter = PXCache<ARCustomerBalanceEnq.ARHistoryFilter>.CreateCopy(instance2.Filter.Current);
          arHistoryFilter.BranchID = header.BranchID;
          arHistoryFilter.IncludeChildAccounts = new bool?(false);
          arHistoryFilter.ShowWithBalanceOnly = new bool?(false);
          arHistoryFilter.SplitByCurrency = new bool?(false);
          foreach (KeyValuePair<DiscrepancyByAccountEnqResultKey, DiscrepancyByAccountEnqResult> keyValuePair in dictionary)
          {
            ...
            arHistoryFilter.Period = keyValuePair.Key.FinPeriodID;
            arHistoryFilter.ARAcctID = keyValuePair.Key.AccountID;
            arHistoryFilter.ARSubID = keyValuePair.Key.SubID;
            arHistoryFilter = instance2.Filter.Update(arHistoryFilter);
            foreach (PXResult<ARCustomerBalanceEnq.ARHistoryResult> pxResult in instance2.History.Select())
            {
              ARCustomerBalanceEnq.ARHistoryResult arHistoryResult = (ARCustomerBalanceEnq.ARHistoryResult) pxResult;
              DiscrepancyByAccountEnqResult accountEnqResult5 = accountEnqResult4;
              Decimal? xxTurnover = accountEnqResult5.XXTurnover;
              Decimal? nullable2 = arHistoryResult.Balance;
              Decimal valueOrDefault1 = nullable2.GetValueOrDefault();
              nullable2 = arHistoryResult.Deposits;
  ...
  {
    ARGLDiscrepancyEnqFilter current1 = this.Filter.Current;
    DiscrepancyByAccountEnqResult current2 = this.Rows.Current;
    if (current1 != null && current2 != null)
    {
      ARGLDiscrepancyByCustomerEnq instance = PXGraph.CreateInstance<ARGLDiscrepancyByCustomerEnq>();
