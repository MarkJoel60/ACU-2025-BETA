...
public sealed class CurrentMatch<Table, Field> : 
  BqlChainableConditionLite<CurrentMatch<Table, Field>>,
  IBqlUnary,
  IBqlCreator,
  IBqlVerifier
  ...
  where Field : IBqlField
{
    ...
    BqlCommand.Selection selection)
  {
    ...
    if (GroupHelper.Count == 0 || !graph.Caches[typeof (Table)].Fields.Contains("GroupMask"))
    {
    ...
    foreach (GroupHelper.ParamsPair paramsPair in GroupHelper.GetParams(GroupHelper.GetReferencedType(cach, typeof (Field).Name), typeof (Table), dataValue))
    {
