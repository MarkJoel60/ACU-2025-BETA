...
// Type: PX.Data.Wiki.Parser.Txt.PXIndentRenderer
...
internal class PXIndentRenderer : PXTxtRenderer
{
    ...
    foreach (PXElement child1 in container.Children)
    {
      ...
      switch (child1)
      {
        ...
        case PXIndentElement _:
          string format = this.FormatNewItem(container, resultTxt, num);
          if (resultTxt.Result.Length != 0 && !resultTxt.Result.EndsWith(Environment.NewLine))
            resultTxt.NewLine();
          resultTxt.LinePrefix += this.GetNewPrefix(format);
  ...
  private string GetNewPrefix(string format)
  {
    string newPrefix = " ";
