...
// Type: PX.Data.Wiki.DITA.PXTopicRefBuildDitaElement
...
public class PXTopicRefBuildDitaElement : PXDitaContainer
{
  ...
  public void Settables(DataSet _set) => this._table = _set;

  public void SetParent(StringBuilder ownname, StringBuilder parent, int Number)
  {
    DataRow[] dataRowArray = this._table.Tables["WikiPage"].Select($"Name = '{ownname?.ToString()}.dita'");
    ...
    if (parent.Length <= 0)
      return;
    DataRow dataRow2 = this._table.Tables["WikiPage"].Select($"Name = '{parent?.ToString()}.dita'")[0];
    dataRow1["ParentUID"] = dataRow2["PageID"];
    dataRow2["Folder"] = (object) true;
  }
}
