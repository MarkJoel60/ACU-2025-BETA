...
// Type: PX.Data.Wiki.DITA.PXNoteDitaElement
...
public class PXNoteDitaElement : PXDitaContainer
{
  ...
  public override StringBuilder Read(StringBuilder globalContent1, ExportContext _context)
  {
    ...
    string str;
    if (this.Attributs.TryGetValue("type", out str))
    {
      if (str == "attention")
        globalContent1_1.Append("((({S:Warn}");
      else
        globalContent1_1.Append("((({S:Hint}");
    }
    else
      globalContent1_1.Append("((({S:Hint}");
