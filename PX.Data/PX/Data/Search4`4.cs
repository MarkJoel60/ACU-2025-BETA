...
public sealed class Search4<Field, Where, Aggregate, OrderBy> : 
  SearchBase<Field, BqlNone, Where, Aggregate, OrderBy>,
  IBqlAggregate,
  IBqlCreator,
  IBqlVerifier
  ...
  where OrderBy : IBqlOrderBy, new()
{
  ...
  {
    return (BqlCommand) new Search4<Field, Where2<Where, And<where>>, Aggregate, OrderBy>();
  }

  public override BqlCommand WhereAnd(System.Type where)
  {
    return (BqlCommand) Activator.CreateInstance(typeof (Search4<,,,>).MakeGenericType(typeof (Field), typeof (Where2<,>).MakeGenericType(typeof (Where), typeof (And<>).MakeGenericType(where)), typeof (Aggregate), typeof (OrderBy)));
  }

  public override BqlCommand WhereOr<where>()
  {
    return (BqlCommand) new Search4<Field, Where2<Where, Or<where>>, Aggregate, OrderBy>();
  }

  public override BqlCommand WhereOr(System.Type where)
  {
    return (BqlCommand) Activator.CreateInstance(typeof (Search4<,,,>).MakeGenericType(typeof (Field), typeof (Where2<,>).MakeGenericType(typeof (Where), typeof (Or<>).MakeGenericType(where)), typeof (Aggregate), typeof (OrderBy)));
  }

  public override BqlCommand WhereNot()
  {
    return (BqlCommand) new Search4<Field, PX.Data.Where<Not<Where>>, Aggregate, OrderBy>();
  }
