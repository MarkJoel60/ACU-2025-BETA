...
public class PXPhoneAttribute : PXEventSubscriberAttribute
{
  ...
  protected internal override void SetBqlTable(System.Type bqlTable)
  {
    ...
    lock (((ICollection) PXPhoneAttribute.FieldList).SyncRoot)
    {
      List<string> stringList;
      if (!PXPhoneAttribute.FieldList.TryGetValue(bqlTable, out stringList))
        PXPhoneAttribute.FieldList[bqlTable] = stringList = new List<string>();
  ...
  public static List<string> GetFields(System.Type table)
  {
    ...
    List<string> stringList;
    return PXPhoneAttribute.FieldList.TryGetValue(table, out stringList) ? stringList : new List<string>();
  }
}
