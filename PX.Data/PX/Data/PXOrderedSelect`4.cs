...
// Type: PX.Data.PXOrderedSelect`4
  ...
  where OrderBy : IBqlOrderBy, new()
{
  ...
  public PXOrderedSelect(PXGraph graph)
  {
    this._Graph = graph;
    this.Initialize();
    this.View = new PXView(graph, false, (BqlCommand) new PX.Data.Select<Table, Where, OrderBy>());
  }
  ...
  public PXOrderedSelect(PXGraph graph, Delegate handler)
  {
    this._Graph = graph;
    this.Initialize();
    this.View = new PXView(graph, false, (BqlCommand) new PX.Data.Select<Table, Where, OrderBy>(), handler);
  }
}
