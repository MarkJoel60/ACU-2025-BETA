...
// Type: PX.TM.IsWorkgroupOfContact`1
...
/// <exclude />
public class IsWorkgroupOfContact<Operand> : IBqlComparison, IBqlCreator, IBqlVerifier where Operand : IBqlOperand, new()
{
    ...
    BqlCommand.Selection selection)
  {
    ...
    if (!info.BuildExpression)
      return flag;
    Query q = new Query();
    q[typeof (EPCompanyTreeMember.workGroupID)].From(typeof (EPCompanyTreeMember)).Where(SQLExpressionExt.EQ(Column.SQLColumn(typeof (EPCompanyTreeMember.contactID)), exp1).And(Column.SQLColumn(typeof (EPCompanyTreeMember.active)).EQ((object) 1)));
    exp = exp.In(q);
    return flag;
  }
}
