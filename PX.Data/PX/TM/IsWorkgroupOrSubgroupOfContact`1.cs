...
// Type: PX.TM.IsWorkgroupOrSubgroupOfContact`1
...
/// <exclude />
public class IsWorkgroupOrSubgroupOfContact<Operand> : IBqlComparison, IBqlCreator, IBqlVerifier where Operand : IBqlOperand, new()
{
    ...
    BqlCommand.Selection selection)
  {
    ...
    if (!info.BuildExpression)
      return flag;
    Query q = new Query();
    q.Select<EPCompanyTreeH.workGroupID>().From<EPCompanyTreeH>().Join<EPCompanyTreeMember>().On(SQLExpressionExt.EQ(typeof (EPCompanyTreeH.parentWGID), typeof (EPCompanyTreeMember.workGroupID)).And(typeof (EPCompanyTreeMember.active).EQ((object) 1)).And(SQLExpressionExt.EQ(typeof (EPCompanyTreeMember.contactID), exp1))).Where(new SQLConst((object) 1).EQ((object) 1));
    exp = exp.In(q);
    return flag;
  }
}
